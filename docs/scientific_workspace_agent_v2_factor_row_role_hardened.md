...
For row-role-hardened consensus isolation, unresolved RAW-role status remains a hard stop for ordinary de novo reconstruction. The only exception is after trusted partial SDRF fusion has succeeded: the trusted design has already closed the multiplex sample/channel relation gap, so repository-only fallback rows that are deterministically classified as `single cell` may receive the canonical isolation value only when all accepted single-cell FactorGraph regimes independently adjudicate to the same source-grounded isolation term. Conflicting or unresolved regimes still fail closed.

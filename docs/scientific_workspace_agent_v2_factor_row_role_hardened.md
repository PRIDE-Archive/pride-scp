...
For row-role-hardened workspace-consensus isolation, unresolved RAW-role status remains a hard stop, including after trusted partial SDRF fusion. Trusted partial fusion handles repository-only fallback rows by deterministic RAW role before validation. Explicit QC tokens such as `cvcheck` remain quality-control rows; explicit mass-amount inputs and `booster` auxiliary runs remain non-single-cell bulk controls; blanks and few-cell inputs retain their ordinary hardened roles. Only a fallback RAW independently classified as `single cell` may receive an isolation value from the deposited design, and only when the deposited SDRF's mapped `single cell` rows contain exactly one explicit non-placeholder `characteristics[single cell isolation protocol]` value. Unknown fallback roles are reset to unresolved rather than inheriting the project-level single-cell scaffold. No FactorGraph-claim, channel-order, or filename-derived channel inference is used.

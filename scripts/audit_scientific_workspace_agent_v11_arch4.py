#!/usr/bin/env python3
"""Audit the frozen arch4 Scientific Workspace Agent v1.1 command-loop run.
...
V10 = {
    ...
    "total_agent_turns": 30,
    "total_tool_actions": 0,
    "total_validator_cycles": 9,
}
...
def summarize_gates(summary: dict[str, Any], rows: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    for row in by_acc.values():
        ...
        total_command_reads += int(row.get("command_counts", {}).get("read_evidence", 0))
        total_command_searches += int(row.get("command_counts", {}).get("search_evidence", 0))
    ...
    return {
        ...
        "validation_errors_below_v10": int(summary.get("total_validation_errors", 10**9))
        < V10["total_validation_errors"],
    }
...
def main() -> int:
    ...
    comparison = {
        key: {
            "v1.0": old,
            "v1.1": int(summary.get(key, 0)),
            "delta_v11_minus_v10": int(summary.get(key, 0)) - old,
        }
        for key, old in V10.items()
    }
    report = {
        ...
        "summary": summary,
        "result_rows": results,
        "comparison_to_v10": comparison,
        "decision_observables": summarize_gates(summary, per_accession),
        "per_accession": per_accession,
        "interpretation_rule": (
            "v1.1 tests the explicit command protocol and scientific-observation/Rust-canonicalization "
            "boundary. At least one real read/search action must execute before a model-capacity "
            "comparison is considered informative. Preserve the PXD025634 template gap and PXD046467 "
            "branch/linkage safety even if validation totals do not improve."
        ),
    }

    print("===== SCIENTIFIC WORKSPACE AGENT v1.1 ARCH4 =====")
    ...
    for key, values in comparison.items():
        print(
            f"{key}\tv1.0={values['v1.0']}\tv1.1={values['v1.1']}"
            f"\tdelta={values['delta_v11_minus_v10']:+d}"
        )
    print("\n===== v1.1 COMMAND / SAFETY GATES =====")

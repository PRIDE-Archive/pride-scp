#!/usr/bin/env python3
"""Audit the frozen arch4 Scientific Workspace Agent v1.2 finite-state command-loop run.
...
V11 = {
    ...
    "total_agent_turns": 48,
    "total_tool_actions": 56,
...
COMMANDS = {"read_evidence", "search_evidence", "edit_workspace", "escalate"}
...
def audit_one(root: Path, accession: str) -> dict[str, Any]:
    ...
    if accession == "PXD035339":
        ...
        out["isolation_observations_final"] = [
            observation
            for observation in observations
...
def summarize_gates(summary: dict[str, Any], rows: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    for row in by_acc.values():
        ...
        total_command_edits += int(counts.get("edit_workspace", 0))
    ...
    return {
        ...
        "edit_workspace_commands": total_command_edits,
        ...
        "at_least_one_edit_or_escalation": (total_command_edits + total_command_escalations) > 0,
        ...
        "validation_errors_below_v11": int(summary.get("total_validation_errors", 10**9))
        < V11["total_validation_errors"],
    }
...
def main() -> int:
    ...
    comparison = {
        key: {
            "v1.1": old,
            "v1.2": int(summary.get(key, 0)),
            "delta_v12_minus_v11": int(summary.get(key, 0)) - old,
        }
        for key, old in V11.items()
    }
    report = {
        ...
        "summary": summary,
        "result_rows": results,
        "comparison_to_v11": comparison,
        "decision_observables": summarize_gates(summary, per_accession),
        "per_accession": per_accession,
        "interpretation_rule": (
            "v1.2 tests the finite-state read->decision control loop. Repeated reads must be blocked by "
            "schema/state, requested refs must be recorded as consumed, and the cohort must progress to "
            "at least one workspace edit or escalation. Preserve template-gap, branch/linkage, and structural safety."
        ),
    }

    print("===== SCIENTIFIC WORKSPACE AGENT v1.2 ARCH4 =====")
    ...
    for key, values in comparison.items():
        print(
            f"{key}\tv1.1={values['v1.1']}\tv1.2={values['v1.2']}"
            f"\tdelta={values['delta_v12_minus_v11']:+d}"
        )
    print("\n===== v1.2 FINITE-STATE / SAFETY GATES =====")

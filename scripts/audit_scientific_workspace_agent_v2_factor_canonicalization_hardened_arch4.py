...
EXPECTED_HARNESS = "pride-scp-scientific-workspace-agent-v2-factor-canonicalization-hardened"
...
def norm_phrase(value) -> str:
    text = low(value)
    return " ".join("".join(ch if ch.isalnum() else " " for ch in text).split())


def isolation_canonicalization_faithful(record: dict) -> bool:
    if record.get("concept_type") != "isolation_method":
        return True
    if adjudication_outcome(record) != "canonical":
        return True
    adj = record.get("adjudication", {})
    observed = norm_phrase(record.get("observed_value"))
    canonical = norm_phrase(adj.get("value"))
    ...
    if canonical == "manual picking":
        return any(term in observed for term in (
            "manual picking", "manual pick", "manually picked",
            "picked single cell", "single cell picked", "cell picking"
        ))
    return False
...
def summarize(output: Path, accession: str) -> dict:
    root = output / "canonicalization_hardened" / accession
    ...
    draft_path = Path(result.get("draft_path", "")) if result.get("draft_path") else None
    draft_text = draft_path.read_text(errors="replace") if draft_path and draft_path.is_file() else ""
    return {
        "accession": accession,
        "present": True,
        ...
        "draft_contains_manual_picking": "manual picking" in low(draft_text),
    }
...
def main() -> int:
    ...
    summary_path = args.output / "factor_canonicalization_hardened_summary.json"
    ...
    p35339_isolation_canonical_fidelity = all(
        isolation_canonicalization_faithful(rec)
        for rec in p35339.get("adjudications", [])
    )
    p35339_no_false_manual_picking = not any(
        ...
        and rec.get("branch_id") in {"R001", "R002"}
        for rec in p35339.get("adjudications", [])
    )
    ...
    all_isolation_canonicalizations_faithful = all(
        isolation_canonicalization_faithful(rec)
        for row in details
        for rec in row.get("adjudications", [])
    )
    no_false_manual_picking_serialized = all(
        not row.get("draft_contains_manual_picking", False)
        for row in details
    )
    ...
    for row in details:
        ...
        validation_deltas[acc] = {
            ...
            "canonicalization_hardened": current,
            "delta": delta,
        }
    ...
    safety_gate = (
        ...
        and p35339_isolation_canonical_fidelity
        and p35339_no_false_manual_picking
        and p46467_no_hela_isolation
        and p25634_no_manual_picking_canonical
        and all_isolation_canonicalizations_faithful
        and no_false_manual_picking_serialized
    )
    progress_gate = (
    ...
    audit = {
        ...
        "decision_observables": {
            ...
            "bounded_canonicalization_hardened_execution": bounded,
            "accepted_factor_graphs_frozen": factor_graphs_frozen,
            "canonicalization_hardened_harness_identity_held": bridge_identity_held,
            ...
            "p35339_isolation_canonicalization_faithful": p35339_isolation_canonical_fidelity,
            "p35339_no_false_manual_picking_canonical": p35339_no_false_manual_picking,
            ...
            "all_isolation_canonicalizations_semantically_faithful": all_isolation_canonicalizations_faithful,
            "no_false_manual_picking_serialized": no_false_manual_picking_serialized,
            ...
            "factor_canonicalization_hardened_safety_gate_passed": safety_gate,
            "factor_canonicalization_hardened_progress_gate_passed": progress_gate,
    ...
    print("===== PRIDE-SCP v2 FACTOR CANONICALIZATION HARDENED — ARCH4 =====")

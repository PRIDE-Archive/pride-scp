...
def summarize(output: Path, accession: str) -> dict:
    root = output / "phase_b" / accession
    result_path = root / "result.json"
    graph_path = root / "accepted_factor_graph.json"
    accepted_path = root / "accepted_observations.json"
    ...
    return {
        ...
        "open_questions": acceptance.get("open_questions", []),
...
def main() -> int:
    ...
    summary_path = args.output / "factor_phase_b_summary.json"
    if not summary_path.is_file():
        raise SystemExit(f"missing factor Phase-B summary: {summary_path}")
    ...
    one_model_call_each = all(row.get("model_calls") == 1 for row in details)
    ...
    for row in details:
        ...
        for obs in row.get("observations", []):
            ...
            rust_factor_ids_only &= bool(target) and target[0] in "MRA" and len(target) == 4 and target[1:].isdigit()

    p35339 = by_acc["PXD035339"]
    p35339_hydrodynamic = has_obs(p35339, "R002", "hydrodynamic") or has_obs(p35339, "R002", "manual")
    p35339_spray = has_obs(p35339, "R001", "spray")
    p35339_no_cross_scope = not (
        has_obs(p35339, "R001", "hydrodynamic")
        or has_obs(p35339, "R002", "spray")
    )
    p35339_scoped = p35339_hydrodynamic and p35339_spray and p35339_no_cross_scope

    p46467 = by_acc["PXD046467"]
    p46467_capillary = (
        has_obs(p46467, "R002", "capillary")
        or has_obs(p46467, "R002", "aspirat")
        or has_obs(p46467, "R002", "microsampl")
    )
    p46467_no_hela_isolation = not any(
        obs.get("target_factor_id") == "R001" and obs.get("concept_type") == "isolation_method"
    ...
    p25634_template_gap = any(
        rec.get("concept_type") == "isolation_method" and adjudication_outcome(rec) == "template_gap"
    ...
    p25634_materials_preserved = len(graph25634.get("materials", [])) >= 2
    p25634_scoped = (
        p25634_microwell
    ...
    for row in details:
        ...
        validation_deltas[acc] = {"historical_v13": baseline, "phase_b": current, "delta": delta}
    ...
    factor_graphs_frozen = all(
        ...
        and row.get("factor_graph_harness") == "pride-scp-scientific-workspace-agent-v2-factor-stage1"
        for row in details
    )
    safety_gate = (
        all_present
    ...
    progress_gate = (
        summary.get("harness_version") == "pride-scp-scientific-workspace-agent-v2-factor-phase-b"
    ...
    audit = {
        ...
        "decision_observables": {
            ...
            "one_observation_bundle_call_per_accession": one_model_call_each,
            "one_validator_pass_per_accession": one_validator_each,
            "no_conversational_tool_loop": no_tool_loop,
            "bounded_phase_b_execution": bounded,
            ...
            "factor_phase_b_safety_gate_passed": safety_gate,
            "factor_phase_b_progress_gate_passed": progress_gate,
    ...
    print("===== SCIENTIFIC WORKSPACE AGENT v2 FACTOR PHASE B — ARCH4 =====")

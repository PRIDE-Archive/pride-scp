...
def repair_019515(rows, changes):
    for row in rows:
        ...
        if "Blank" in fn:
            ...
            setv(row, "characteristics[single cell isolation protocol]", "laser capture microdissection", changes,
                 "publication: neurons excised by LCM")
        # The publication states that the selected two-CV method used HCD fragmentation.
...
def repair_019958(rows, changes):
    for row in rows:
        ...
        # Zero-cell controls contain no biological cell. Do not project U-87 MG / astrocyte / brain
        # identity onto the empty control merely because it belongs to the same preparation series.
        ...
        if role in {"empty", "blank", "negative control"} or cells == "0":
            for h in (
                ...
                setv(row, h, "not applicable", changes, "zero-cell control has no biological cell identity")
    ...
    for row in rows:
        ...
        if hela:
            ...
            if "DDA" in fn:
                ...
                setv(row, "comment[proteomics data acquisition method]", "NT=Data-independent acquisition;AC=PRIDE:0000450", changes,
                     "WWA wide-window reference acquisition")
                setv(row, "comment[instrument]", "NT=Orbitrap Fusion Lumos;AC=MS:1002732", changes,
                     "publication: tribrid WWA reference on Fusion Lumos")
        else:

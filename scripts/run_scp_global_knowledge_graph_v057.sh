...
ROOT="${ROOT:-$PWD}"
OUT="${OUT:-$ROOT/data/scp_global_knowledge_graph_v057}"
...
{
  "phase": "SCP_global_knowledge_graph_v057_optimized_small_llm_semantic_claims",
  "graph_version": "pride-scp-global-knowledge-graph-v0.2",
  "resolver_version": "pride-scp-kg-resolver-v0.2",
  "semantic_extractor_version": "pride-scp-kg-small-llm-semantic-extractor-v0.2",
  "architectural_changes": [
    ...
    "reuse compatible completed v0.5.6 packet caches by source hash/locator even across prompt-version changes",
...
if [[ "$RESUME" == "1" && -f "$DB" ]]; then
  echo "resume mode: reusing existing v0.5.7 base graph: $DB"
  BASE_READY=1
...
printf '\nv0.5.7 optimized global SCP knowledge graph + small-LLM semantic extraction complete (non-generative)\n'
printf '  database:           %s\n' "$DB"
